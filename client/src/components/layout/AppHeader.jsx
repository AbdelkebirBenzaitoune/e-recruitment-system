...
function UserProfileModal({ user, onClose, onUpdateUser }) {
  ...
  // Nouvelle fonction pour charger le profil complet
  const loadFullUserProfile = async () => {
    try {
      const result = await userApiService.getUserProfile();
      // Met à jour les données du modal avec le profil complet
      setProfileData({
      ...
      console.error('Erreur rechargement profil:', error);
      // En cas d'erreur, garde les données actuelles
      setError('Erreur lors du chargement du profil');
    }
  };

  // Charger le profil complet à l'ouverture du modal
  React.useEffect(() => {
    loadFullUserProfile();
  }, []); // Charge une seule fois à l'ouverture
  ...
  async function handleUpdateUser(newProfileData) {
    try {
      ...
      // Recharge le profil depuis le serveur pour s'assurer de la cohérence
      setTimeout(async () => {
        try {
          await loadFullUserProfile();
        } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Met à jour avec les données retournées par l'API
        const updatedUser = result.updatedUser || { ...user, ...profileData };
